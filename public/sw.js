...
const CACHE_NAME = 'bahati-pro-1.0.15-601a5d0';
...
self.addEventListener('fetch', (event) => {
  ...
  if (
    ...
    event.respondWith(
      fetch(event.request)
        .then((response) => {
          // Only cache successful responses — redirects (302) and errors
          // (4xx/5xx) must not be cached or they produce a broken offline UX.
