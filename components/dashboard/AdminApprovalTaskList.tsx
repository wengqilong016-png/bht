    ...
    <div className="space-y-3">
      {approvalTasks.length > 0 && (
        <div className="flex items-center gap-2 px-1">
          ...
          <p className="text-caption font-bold text-[#a09080] uppercase tracking-widest truncate">
            {[
      ...
      {approvalTasks.length === 0 ? (
        <div className="py-10 text-center bg-white rounded-2xl border border-dashed border-[#e0d8cc]">
          <CheckCircle2 size={40} className="mx-auto text-emerald-200 mb-3" />
          <p className="text-xs font-bold text-[#a09080] uppercase tracking-widest">{t.allSettlementsProcessed}</p>
        </div>
      ) : (
                ...
                >
                  ...
                  <div className="min-w-0 flex-1">
                    <div className="flex items-center gap-1.5 flex-wrap">
                      ...
                      <span className="text-caption font-black text-[#171310] truncate">{task.driverName}</span>
                    </div>
                    <p className="text-caption font-bold text-[#a09080] truncate mt-0.5">{task.locationName} · {new Date(task.timestamp).toLocaleDateString(lang === 'zh' ? 'zh-CN' : 'en-GB')}</p>
                  </div>
                  <div className="flex-shrink-0 flex items-center gap-2">
                    <span className="text-caption font-black text-[#3d3028]">
                  ...
                  <div className="mt-3 pt-3 border-t border-[#e8e0d4] space-y-3">
                    {task.type === 'settlement' && (() => {
                      ...
                      return (
                        <>
                          <div className="space-y-2">
                            <p className="text-caption font-black text-[#a09080] uppercase">
                              {t.dailySummaryLabel}
                            </p>
                            <div className="grid grid-cols-2 gap-2">
                              <div className="bg-[#f3efe8] p-3 rounded-xl">
                                <p className="text-caption font-black text-[#a09080] uppercase">{t.expectedTotalLabel}</p>
                                <p className="text-xs font-bold text-[#171310]">TZS {s.expectedTotal.toLocaleString()}</p>
                              </div>
                              <div className="bg-amber-50 p-3 rounded-xl">
                                <p className="text-caption font-black text-amber-500 uppercase">{t.actualSubmittedLabel}</p>
                                <p className="text-xs font-bold text-amber-700">TZS {(s.actualCash + s.actualCoins).toLocaleString()}</p>
                              </div>
                              <div className="bg-[#f3efe8] p-3 rounded-xl">
                                <p className="text-caption font-black text-[#a09080] uppercase">{t.inputCash}</p>
                                <p className="text-xs font-bold text-[#171310]">TZS {s.actualCash.toLocaleString()}</p>
                              </div>
                              <div className="bg-[#f3efe8] p-3 rounded-xl">
                                <p className="text-caption font-black text-[#a09080] uppercase">{t.inputCoins}</p>
                                <p className="text-xs font-bold text-[#171310]">TZS {s.actualCoins.toLocaleString()}</p>
                              </div>
                            </div>
                          </div>
                          ...
                          <div className="grid grid-cols-2 gap-2">
                            <div className="bg-[#f3efe8] p-3 rounded-xl">
                              <p className="text-caption font-black text-[#a09080] uppercase">{t.collectionsCount}</p>
                              <p className="text-xs font-bold text-[#171310]">{collectionCount.toLocaleString()}</p>
                            </div>
                            <div className="bg-[#f3efe8] p-3 rounded-xl">
                              <p className="text-caption font-black text-[#a09080] uppercase">{t.revenueLabel}</p>
                              <p className="text-xs font-bold text-[#171310]">TZS {s.totalRevenue.toLocaleString()}</p>
                            </div>
                            <div className="bg-[#f3efe8] p-3 rounded-xl">
                              <p className="text-caption font-black text-[#a09080] uppercase">{t.publicExp}</p>
                              <p className="text-xs font-bold text-[#171310]">TZS {s.totalExpenses.toLocaleString()}</p>
                            </div>
                            <div className="bg-[#f3efe8] p-3 rounded-xl">
                              <p className="text-caption font-black text-[#a09080] uppercase">{t.cashInHand}</p>
                              <p className="text-xs font-bold text-[#171310]">TZS {s.totalNetPayable.toLocaleString()}</p>
                            </div>
                          </div>
                          {s.shortage !== 0 && (
                            <div className={`p-3 rounded-xl flex items-center justify-between ${s.shortage < 0 ? 'bg-rose-50 text-rose-600' : 'bg-emerald-50 text-emerald-600'}`}>
                              <span className="text-caption font-bold uppercase">{s.shortage < 0 ? t.shortage : t.surplus}</span>
                              <span className="text-xs font-bold">TZS {Math.abs(s.shortage).toLocaleString()}</span>
                            </div>
                          )}
                          {(s as unknown as { transferProofUrl?: string }).transferProofUrl && (
                            <img src={(s as unknown as { transferProofUrl: string }).transferProofUrl} alt={t.settlementProof} className="w-full h-28 object-cover rounded-xl border border-[#e0d8cc]" />
                          )}
                          <div className="flex gap-2">
                            ...
                            <button aria-label={t.rejectBtn} type="button" disabled={isPending || !isOnline} onClick={() => runApprovalAction(task.key, () => onReviewSettlement(task.id, 'rejected'))} className="flex-1 py-3 bg-[#ede6dc] text-[#a09080] rounded-xl text-caption font-black uppercase disabled:opacity-50">✗ {t.rejectBtn}</button>
                          </div>
                          <p className="text-caption font-bold leading-relaxed text-[#8c7e6d]">
                            {lang === 'zh'
                      ...
                      return (
                        <>
                          <div className="flex items-center justify-between">
                            <div>
                              <p className="text-caption font-bold text-[#8c7e6d]">{categoryLabel}</p>
                              <p className="text-xs font-bold text-[#171310]">TZS {tx.expenses.toLocaleString()}</p>
                            </div>
                          ...
                          <div className="flex gap-2">
                            ...
                            <button aria-label={t.rejectBtn} type="button" disabled={isPending || !isOnline} onClick={() => runApprovalAction(task.key, () => onApproveExpenseRequest(task.id, false))} className="flex-1 py-2.5 bg-[#ede6dc] text-[#a09080] rounded-xl text-caption font-black uppercase disabled:opacity-50">✗ {t.rejectBtn}</button>
                          </div>
                          <p className="text-caption font-bold leading-relaxed text-[#8c7e6d]">
                        ...
                        <>
                          <div className="grid grid-cols-2 gap-2">
                            <div className="bg-[#f3efe8] p-2 rounded-xl">
                              ...
                              <p className="text-xs font-bold text-[#171310]">{tx.currentScore}</p>
                            </div>
                            <div className="bg-amber-50 p-2 rounded-xl">
                              ...
                              <p className="text-xs font-bold text-amber-700">
                                ...
                                {scan?.status === 'unclear' && <span className="text-[#a09080]">{lang === 'zh' ? '不清晰' : 'Unclear'}</span>}
                                {scan?.status === 'error' && <span className="text-[#a09080]">{tx.aiScore ?? 'N/A'}</span>}
                                {!scan && (tx.aiScore ?? 'N/A')}
                              </p>
                            </div>
                          </div>
                          ...
                          {scan?.notes && (
                            <p className="text-caption font-bold text-[#8c7e6d] leading-relaxed">
                              {scan.notes}
                            </p>
                          )}
                          {tx.photoUrl && (
                            showThumbnail
                              ...
                              : <img src={getOptimizedImageUrl(tx.photoUrl, 400, 300)} alt={t.paymentProof} className="w-full h-24 object-cover rounded-xl border border-[#e0d8cc]" />
                          )}
                          <div className="flex gap-2">
                    ...
                    {task.type === 'reset' && (() => {
                      ...
                      return (
                        <>
                          <div className="bg-[#f3efe8] p-3 rounded-xl">
                            <p className="text-caption font-black text-[#a09080] uppercase">{lang === 'zh' ? '重置前当前分数' : 'Current Score (Before Reset)'}</p>
                            <p className="text-lg font-black text-amber-700">{tx.currentScore}</p>
                            {loc?.machineId && <p className="text-caption font-bold text-[#a09080] mt-0.5">{loc.machineId}</p>}
                          </div>
                          {tx.photoUrl && (
                            <img src={getOptimizedImageUrl(tx.photoUrl, 400, 300)} alt={t.paymentProof} className="w-full h-28 object-cover rounded-xl border border-[#e0d8cc]" />
                          )}
                          <div className="flex gap-2">
                            ...
                            <button aria-label={t.rejectBtn} type="button" disabled={isPending || !isOnline} onClick={() => runApprovalAction(task.key, () => onApproveResetRequest(task.id, false))} className="flex-1 py-2.5 bg-[#ede6dc] text-[#a09080] rounded-xl text-caption font-black uppercase disabled:opacity-50">✗ {t.rejectBtn}</button>
                          </div>
                        </>
                      );
                    })()}
                      ...
                      return (
                        <>
                          <div className="bg-emerald-50 p-3 rounded-xl text-center">
                            ...
                            <p className="text-caption font-bold text-[#a09080] mt-1">{t.availableBalance}: TZS {(loc?.dividendBalance || 0).toLocaleString()}</p>
                            {loc?.ownerName && <p className="text-caption font-bold text-[#a09080]">{t.ownerLabel}: {loc.ownerName}</p>}
                          </div>
                          <div className="grid grid-cols-2 gap-2">
                            <div className="rounded-xl bg-[#f3efe8] p-3">
                              <p className="text-caption font-black uppercase text-[#a09080]">
                                {lang === 'zh' ? '当前点位余额' : 'Current location balance'}
                              </p>
                              <p className="text-xs font-bold text-[#171310]">TZS {(loc?.dividendBalance || 0).toLocaleString()}</p>
                            </div>
                          ...
                          <div className="flex gap-2">
                            ...
                            <button disabled={isPending || !isOnline} onClick={() => runApprovalAction(task.key, () => onApprovePayoutRequest(task.id, false))} className="flex-1 py-2.5 bg-[#ede6dc] text-[#a09080] rounded-xl text-caption font-black uppercase disabled:opacity-50">✗ {t.rejectBtn}</button>
                          </div>
                        </>
                      );
                    })()}
