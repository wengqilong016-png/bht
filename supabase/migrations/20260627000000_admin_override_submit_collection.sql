-- Migration: allow admin override for submit_collection_v2
-- When p_admin_override = TRUE:
--   1. Allow negative score diff (skip GREATEST(0, ...) clamp)
--   2. Allow lastScore to decrease (update regardless of comparison)
--   3. Skip amount validation gate for currentScore
--
-- This enables:
--   - Rollover corrections (counter wrapped around to 0)
--   - Admin error corrections (wrong previous score was recorded)
...
BEGIN
    ...
    -- Only admins can use override
    IF p_admin_override AND v_caller_profile.role <> 'admin' THEN
        RAISE EXCEPTION 'Only admins may use admin override'
            USING ERRCODE = '42501';
    END IF;

    -- ─── AMOUNT VALIDATION GATE ──────────────────────────────────────────────
    -- Admin override skips score ceiling validation (rollover / correction).
    ...
    -- Admin override allows negative diff (rollover / correction).
    IF p_admin_override THEN
        v_diff := p_current_score - v_location."lastScore";
    ELSE
    ...
    IF v_rows_inserted = 1 THEN
        -- Admin override: always update lastScore (allow decrease).
        -- Normal path: only update if new score >= old score.
    ...
    IF v_rows_inserted = 0 THEN
        SELECT
            t.id, t."timestamp",
            t."locationId", t."locationName", t."driverId", t."driverName",
            t."previousScore", t."currentScore",
            t.revenue, t.commission, t."ownerRetention",
            t."debtDeduction", t."startupDebtDeduction",
            t.expenses, t."coinExchange", t."extraIncome", t."netPayable",
            t."paymentStatus", t.gps, t."photoUrl",
            t."aiScore", t."isAnomaly", t."isSynced",
            t.type, t."approvalStatus", t."reportedStatus", t.notes,
            t."expenseType", t."expenseCategory", t."expenseStatus"
        INTO v_existing_tx
